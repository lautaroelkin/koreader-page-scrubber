...
function PageScrubber:_paintToImpl(bb, x, y)
    ...
    local function drawFloatingBtn(btn_id, dimen, tw)
        ...
        if btn_id == "arr_l" or btn_id == "arr_r" then
            ...
            local radius = math.floor(dimen.w / 2)
            paintCircle(bb, cx, cy, radius, Blitbuffer.COLOR_BLACK)
            paintCircle(bb, cx, cy, radius - Screen:scaleBySize(2), bg_color)
            
            tw.fgcolor = fg_color
            local tsz = tw:getSize()
            local opt_x = (btn_id == "arr_l") and Screen:scaleBySize(1) or Screen:scaleBySize(-1)
            local opt_y = -Screen:scaleBySize(2)
            tw:paintTo(bb, cx - math.floor(tsz.w / 2) + opt_x, cy - math.floor(tsz.h / 2) + opt_y)
            return
        end
    end
